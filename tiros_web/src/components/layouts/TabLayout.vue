<template>
  <global-layout @dynamicRouterShow="dynamicRouterShow">
    ...
    <div :class="{tabPageContent: multipage, singlePageContent: !multipage}">
...
export default {
  ...
  created() {
    if (this.$route.path != indexKey && this.$route.path != INDEX_MAIN_PAGE_PATH) {
      ...
      if (!newRoute.meta.hidden) {
        ...
        if (!this.multipage) {
          ...
          if (newRoute.meta.keepAlive) {
            // change by Jakgong : 注释掉，因为新打开页面，导致其他页面重新加载
            // this.routeReload()
          }
  ...
  methods: {
    ...
    changeTitle(title) {
      let projectTitle = window._CONFIG['systemName']
      // 首页特殊处理
...
<style lang="less">
body.led-screen-client .singlePageContent{
  padding: 0px;
...
.tab-layout-tabs.ant-tabs {
  ...
  border-left: 1px solid #fcf8f8 !important;
