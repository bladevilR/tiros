<template>
  ...
  >
    <div style='padding: 0px 20px'>
      <a-form-model ref='outPartForm' :model='modalInfo' :rules='modalRules' :label-col='{span:5}'
                    :wrapper-col='{span:18}'>
        ...
        <a-form-model-item :label-col='labelCol' :wrapper-col='wrapperCol' v-if="modalInfo.facadeStatus === '1'"
                           label='外观描述' prop='facadeDesc'>
        ...
        <a-form-model-item :label-col='labelCol' :wrapper-col='wrapperCol' v-if="modalInfo.mixtoolStatus === '1'"
                           label='工装描述' prop='mixtoolDesc'>
          <a-input v-model='modalInfo.mixtoolDesc'></a-input>
        </a-form-model-item>
        <!--<a-form-model-item label="存在故障" prop="fault">
    ...
    <!--    >-->
    <!--    </CarDeviceSelectNew>-->
    <out-asset-list ref='assetSelectModal' @ok='onSelectAsset'></out-asset-list>
  </a-modal>
</template>

<script>
import { randomUUID } from '@/utils/util'
import OutAssetList from '@views/tiros/group/myWork/OutAssetList'
// import CarDeviceSelectNew from "@views/tiros/common/selectModules/CarDeviceSelectNew";

export default {
  name: 'OutPartModal',
  // components:{ CarDeviceSelectNew},
  components: { OutAssetList },
  props: {
    title: {
      type: String,
      default: '部件添加'
  ...
  methods: {
      ...
      this.$refs.outPartForm.validate((valid) => {
        if (valid) {
          if (this.checkAssetIsExist(this.modalInfo.turnoverAssetId)) {
            this.$message.warning('你选择的设备已经存在委外设备列表中,不能重复添加')
            return
          }
          this.$emit('ok', this.modalInfo)
          this.close()
        }
      })
    },
    ...
    selectAsset() {
      ...
      //   return
      // }
      // this.$refs.assetSelectModal.showModal(this.modalInfo.trainNo, this.structureId)
      this.$refs.assetSelectModal.showModal(this.modalInfo.trainNo)
    ...
    onSelectAsset(assets) {
      if (assets && assets.length > 0) {
        ...
        this.modalInfo.outinId = ''
        this.modalInfo.assetNo = asset.assetNo // structure.code
        this.modalInfo.assetName = asset.assetName
        this.modalInfo.amount = asset.amount
        this.modalInfo.assetTypeId = asset.assetTypeId
        this.modalInfo.assetTypeName = asset.assetTypeName
        this.modalInfo.trainNo = asset.trainNo
        this.modalInfo.turnoverAssetId = asset.turnoverAssetId
        this.modalInfo.facadeStatus = asset.facadeStatus
        this.modalInfo.facadeStatus_dictText = this.modalInfo.facadeStatus === '1' ? '有问题' : '无问题'
        this.modalInfo.facadeDesc = asset.facadeDesc
        this.modalInfo.mixtoolStatus = asset.mixtoolStatus
        this.modalInfo.mixtoolStatus_dictText = this.modalInfo.mixtoolStatus === '1' ? '有问题' : '无问题'
        this.modalInfo.mixtoolDesc = asset.mixtoolDesc
        this.modalInfo.fault = asset.fault
        this.modalInfo.fault_dictText = this.modalInfo.fault === '1' ? '是' : '否'
        this.modalInfo.returnStatus = 1
        this.modalInfo.returnStatus_dictText = '是'
        this.modalInfo.returnTime = asset.returnTime
        this.modalInfo.delayReason = asset.delayReason
        this.modalInfo.remark = asset.remark
        this.modalInfo.outDetailId = asset.outDetailId
        this.modalInfo.qualityDate = asset.qualityDate
        this.modalInfo.qualityDay = asset.qualityDay
        this.$forceUpdate()
