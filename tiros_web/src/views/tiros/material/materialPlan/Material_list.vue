  ...
  <div class="bodyWrapper">
    ...
    <div class="table-page-search-wrapper">
      <a-form layout="inline">
          ...
          <a-col :md="4" :sm="24">
            <a-form-item label="提报人">
              <a-select
                ...
                :showArrow="true"
                allowClear
                @change="sbUserNameChange"
                @dropdownVisibleChange="openUserSelectModal"
                ref="myUserSelect"
              >
        ...
        <a-row>
          <a-col :md="4" :sm="24">
            <a-form-item label="维修车辆数">
              <a-input-number :min="1" :step="1" :precision="0"  v-model="queryParam.materialYearTrain"> </a-input-number>
            </a-form-item>
          </a-col>
          <a-col :md="19" :sm="24" :offset="1">
          <span style="float: left; overflow: hidden;" class="table-page-search-submitButtons">
            <a-space>
              <a-button @click="findList">查询</a-button>
              <a-button type="primary" @click="handleAdd">新增</a-button>
...
import {
  ...
  deleteMaterialPlanItem, getMaterialYearTrain
...
export default {
  ...
  created() {
    this.getMaterialYearTrain()
  },
  methods: {
    sbUserNameChange(value){
      if(!value){
        this.$set(this.queryParam, 'sbUserId', '')
      }
    },
    // 弹出日历和关闭日历的回调
    ...
    getMaterialYearTrain(){
      getMaterialYearTrain().then((res) => {
        if(res.success){
          this.queryParam.materialYearTrain=res.result
        }
        this.findList()
      })
    },
