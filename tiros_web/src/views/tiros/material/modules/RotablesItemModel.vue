        ...
        <a-row :gutter="24">
          ...
          <a-col :md="8" :sm="24">
            <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="物资编码">
              <a-input placeholder="请输入" disabled  v-decorator="[ 'materialCode', validatorRules.materialCode]"/>
            </a-form-item>
          </a-col>
          <a-col :md="8" :sm="24">
          ...
          <a-col :md="8" :sm="24">
            <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="保管人员">
              <a-select
                ...
                :open="false"
                :showArrow="true"
                v-decorator="[ 'dutyUserName', validatorRules.dutyUserName ]"
                @focus="openuserModal"
                ref="myuserSelect"
              >
                <a-icon slot="suffixIcon" type="ellipsis" />
        ...
        <a-row :gutter="24">
          ...
          <a-col :md="8" :sm="24">
            <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="出库日期" width="40">
              <a-date-picker v-decorator="[ 'outDate', validatorRules.outDate]" />
            </a-form-item>
          </a-col>
          <a-col :md="8" :sm="24">
...
<script>
  ...
  export default {
    name: 'RotablesItemModel',
    components: { MaterialList, UserList, TrainAssetType,LineSelectList },
    data() {
      return {
        dictCodeStr:'bu_mtr_workshop_group,group_name,id,workshop_id=\'' + this.$store.getters.userInfo.workshopId + '\'|workshop_id',
        value: 1,
        title: '操作',
        visible: false,
        model: {},
    ...
    methods: {
      changeWorkShop(data){
        console.log((data))
        if(data){this.dictCodeStr='bu_mtr_workshop_group,group_name,id,workshop_id='+data}
        else{
          this.dictCodeStr='bu_mtr_workshop_group,group_name,id,workshop_id=\'' + this.$store.getters.userInfo.workshopId + '\'|workshop_id'
        }
      },
      changeClose(checked) {
