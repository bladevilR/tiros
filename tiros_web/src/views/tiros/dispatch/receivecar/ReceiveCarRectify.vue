<template>
  <div class="bodyWrapper">
    ...
    <div :style="`height: calc(100vh - ${isReadonly ? '134' : '240'}px)`">
      ...
      >
        ...
        <vxe-table-column
          ...
          header-align="center"
        ></vxe-table-column>
        <vxe-table-column field="changeDesc" title="整改内容描述" min-width="120"></vxe-table-column>
        <vxe-table-column field="deptName" title="责任部门" min-width="100"></vxe-table-column>
        <vxe-table-column field="dutyManName" title="责任人" width="100"></vxe-table-column>
        <vxe-table-column field="changeDate" title="整改日期" width="100"></vxe-table-column>
        <vxe-table-column field="checkDate" title="检查日期" width="100"></vxe-table-column>
        <vxe-table-column
          field="annex"
          title="附件下载"
          min-width="140"
    ...
    <rectify-modal ref="rectifyModal" @ok="loading = false" :exchangeId="exchangeId" @addItem="addItem"></rectify-modal>
  </div>
</template>

<script>
import { delRectify, getRectifyList } from '@api/tirosDispatchApi'
import RectifyModal from '@views/tiros/dispatch/receivecar/RectifyModal'
import { everythingIsEmpty } from '@/utils/util'
export default {
  name: 'ReceiveCarRectify',
  components: { RectifyModal },
  props: ['exchangeId', 'isReadonly'],
  data() {
    return {
      queryParam: {
        exchangeId: '',
  ...
  methods: {
    findList() {
      if (!everythingIsEmpty(this.exchangeId)) {
        ...
        getRectifyList(this.queryParam).then((res) => {
          ...
          this.$bus.$emit('setReceiveCarRectifyData', this.tableData)
        })
      }
    },
    ...
    handleAdd() {
      this.$refs.rectifyModal.title = '新增'
      this.$refs.rectifyModal.add()
    },
    handleEdit(row, index) {
      this.$refs.rectifyModal.title = '编辑'
      this.$refs.rectifyModal.edit(row, index)
    },
    handleDel() {
      const that = this
