  ...
  >
      ...
      <a-form layout='inline'>
        <a-row :gutter='24'>
          ...
          <a-col :md='6' :sm='8'>
            <a-form-item>
              <a-space>
                <a-button class='primary-color' @click='loadPlanForms'>查询</a-button>
    ...
    <div style='height:calc(100vh - 135px)'>
      ...
      >
        ...
        <vxe-table-column field='remark' title='备注' width='160' placeholder=''
                          :edit-render="{ name: '$input',props:{placeholder:'请输入备注'} }" align='left'
                          header-align='center'>
          <!-- <template v-slot:edit="{ row }">
            <vxe-input v-model="row.remark" placeholder="默认尺寸"></vxe-input>
...
import { addTrainPlanForm, delTrainPlanForm, editTrainPlanForm, listTrainPlanForm } from '@/api/tirosDispatchApi'
...
export default {
  name: 'PlanForms',
  components: { FormsList, TrainAssetType, TrainStructureList, RecordTableView, FormViewModal, JobCheckTableView },
  data() {
    return {
      title: '表单关联管理',
      visible: false,
      ...
      validRules: {
        ...
        remark: [{ max: 200, message: '输入长度不能超过200字符!' }]
      },
      forms: [],
      planId: '',
      trainTypeId: '',
  ...
  methods: {
    ...
    loadPlanForms() {
      ...
      listTrainPlanForm(this.queryParam)
        .then((res) => {
          if (res.success) {
          ...
          delTrainPlanForm('ids=' + ids)
            .then((res) => {
              if (res.success) {
                ...
                console.error('删除计划表单失败：', res.message)
              }
            })
            .catch((err) => {
      ...
      this.forms.map((item2, index2) => {
        if (row.id === item2.id) {
          delTrainPlanForm('ids=' + row.id)
            ...
            .catch((err) => {
              this.$message.error('删除异常')
              console.error('删除计划表单异常：', res.message)
            })
        } else {
          this.$message.error('删除失败')
          console.error('删除计划表单失败：', res.message)
            ...
            addTrainPlanForm(row)
              .then((res) => {
                if (res.success) {
                  ...
                  console.error('保存计划表单失败：', res.message)
                }
              })
              .catch((err) => {
                this.$message.error('保存异常')
                console.error('保存计划表单异常：', err)
              })
          }
          if (this.curEditMode === 2) {
            // 更新
            editTrainPlanForm(row)
              .then((res) => {
                if (res.success) {
                  this.$refs.listTable.clearActived()
                  this.curEditMode = 0
