  ...
  >
    <a-form layout="vertical">
      ...
      <a-form-item label="审核意见">
        <a-textarea v-model="reviewComment" :rows="4" placeholder="请输入审核意见"></a-textarea>
      </a-form-item>
    </a-form>
  </a-modal>
</template>
...
<script>
import { decisionJgbReview } from '@/api/tirosApi'

export default {
  name: 'JobGuideBookReviewModal',
  data () {
    return {
      visible: false,
      loading: false,
  ...
  methods: {
    ...
    handleOk () {
      ...
      if (this.record.reviewStatus !== 1) {
        this.$message.warning('仅待审状态可进行审核')
        return
      }
