<template>
  <a-card id='trainSysLeftContent'>
    ...
    <a-spin :spinning='loading'>
      <div style='width: 70%; margin: 0px auto; margin-top: 15px; margin-bottom: 10px'>
        <j-dict-select-tag
          class='na-display-jselect-clear-btn'
          v-model='trainTypeSelectId'
      ...
      <div style='width: 70%; margin: 0 auto'>
        <a-tree
          ...
          :replaceFields="{ title: 'nameAndShortName', key: 'id' }"
    ...
    <train-type-sys-item-modal
      ref='modalForm'
      @ok='loadData'
      :trainTypeId='trainTypeSelectId'
    ></train-type-sys-item-modal>
  </a-card>
</template>
...
export default {
  name: 'TrainTypeSysCompositionLeft',
  ...
  created() {
    this.queryTreeData()
    this.trainTypeSelectId = this.trainTypeId
  },
  methods: {
    queryTreeData() {
      if (everythingIsEmpty(this.trainTypeSelectId)) {
        return
      }
      trainTypeSysList({
        trainTypeId: this.trainTypeSelectId
      })
        .then((res) => {
          if (res.success) {
            ...
            this.treeDataSource.map((item) => {
              item.nameAndShortName = item.name + '（简称:' + item.shortName + '）'
            })
          }
        })
        .finally(() => {
    ...
    handleTreeSelect(selectedKeys, event) {
      if (selectedKeys.length > 0) {
        ...
        // console.log('this.$emit(\'input\', param)')
        // console.log(param)
        this.$emit('input', param)
    ...
    trainChange(trainTypeId = undefined) {
      if (trainTypeId) {
        this.trainTypeSelectId = trainTypeId
      }
      if (!everythingIsEmpty(this.trainTypeSelectId)) {
        this.queryTreeData()
      }
