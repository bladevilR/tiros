...
<div>
  ...
  >
    ...
    <vxe-table-column field="requirePostion" title="岗位要求" width="300" :edit-render="{name: 'input'}" align="left" header-align="center">
      <template v-slot:edit="{row}">
        <a-input @change="remarkChangeValue(row)" placeholder="请输入岗位需求"  :maxLength="201" v-model="row.requirePostion" />
      </template>
    </vxe-table-column>
    <vxe-table-column field="requireTech" title="技能要求" width="300" :edit-render="{name: 'input'}" align="left" header-align="center">
      <template v-slot:edit="{row}">
        <a-input @change="remarkChangeValue(row)" placeholder="请输入技能要求"  :maxLength="201" v-model="row.requireTech" />
      </template>
    </vxe-table-column>
    <vxe-table-column field="requireCertificate" title="证书要求" width="300" :edit-render="{name: 'input'}" align="left" header-align="center">
      <template v-slot:edit="{row}">
        <a-input @change="remarkChangeValue(row)" placeholder="请输入证书要求"  :maxLength="201" v-model="row.requireCertificate" />
      </template>
    </vxe-table-column>
  ...
  data () {
    return {
      ...
      validRules: {
        ...
        requirePostion:[
          {max:200,type:'string', message:'不能超过200个字符'},
        ],
        requireTech:[
          {max:200,type:'string', message:'不能超过200个字符'},
        ],
