      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a-dropdown>
            ...
            <a-menu slot="overlay">
              <a-menu-item><a @click="executeJob(record)">手动执行</a></a-menu-item>
...
export default {
  ...
  methods: {
    ...
    executeJob: function(record) {
      var that = this
      // 执行一次定时任务
      this.$confirm({
        title: '确认执行',
        content: '是否执行选中任务【' + record.description + '】?',
        onOk: function() {
          getAction(that.url.execute, { jobClassName: record.jobClassName }).then((res) => {
            if (res.success) {
              that.$message.success(res.message)
              that.loadData()
              that.onClearSelected()
