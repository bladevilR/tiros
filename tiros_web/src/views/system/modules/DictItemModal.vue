    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="数据类型">
          <a-select v-decorator.trim="['type', validatorRules.type]" >
        ...
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="排序值">
          <a-input-number :min="0" v-decorator="['sortOrder', { initialValue: 1 }]" />
          值越小越靠前，支持小数
        </a-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      validatorRules: {
        ...
        type: { rules: [{ required: true, message: '请选择数据类型!' }] }
      }
    }
  },
  created() {},
  methods: {
    ...
    edit(record) {
      ...
      this.visible = true
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.model, 'itemText', 'itemValue', 'description', 'sortOrder','type'))
      })
    },
    onChose(checked) {
      if (checked) {
