...
// import uuid from 'uuid'   // 此处引入警告："export 'default' (imported as 'uuid') was not found in 'uuid'  已修改
...
export function everythingIsEmpty (v) {
  switch (typeof v) {
  case 'undefined':
    return true
  case 'string':
...
export function generateIndexRouter (data) {
  ...
  let exist=false;
  rts.forEach(item => {
    if (item.path === INDEX_MAIN_PAGE_PATH) {
      exist=true
      return false
    }
  })
  if (!exist) {
    rts.push({
      ...
      component:  resolve => require(['@/views/tiros/dashboard/EmptyPage.vue'], resolve),
      hidden: false,
      meta: {
        title: '首页',
        icon: 'home',
 ...
 /* {
    path: '/dashboard/index',
      name: 'dashboard-index',
    component: () => import(/!* webpackChunkName: "fail" *!/ '@/views/tiros/dashboard/EmptyPage')
  },
  */
  let indexRouter = [{
    path: '/',
    ...
    children: [
      ///...generateChildRouters(data) 下面为jakgong修改，改成所有路由直接挂在根路由下
      //...generateFlatRouters(data)
      ...rts
    ]
  }, {
    'path': '*', 'redirect': '/404', 'hidden': true
  }]
  // console.log('routes:',indexRouter)
  return indexRouter
}
...
/**
 * 根据时间生成UUID
 * @param splitter  是否添加'-'?  默认不添加
 * @returns {*}
 * @constructor
 */
export function UUID_V1 (splitter) {
  if (splitter) {
    return uuidv1()
  } else {
 ...
 * @constructor
 */
export function UUID_V4 (splitter) {
  if (splitter) {
    return uuidv4()
  } else {
...
export function stringSubByLen (str, len, hasDot) {
  let newLength = 0;
  let newStr = "";
...
//解决js运算精度丢失问题  减法
export function jsAdd (arg1, arg2) {
  var r1, r2, m;
...
//解决js运算精度丢失问题  减法
export function jsCut (arg1, arg2) {
  var r1, r2, m, n;
  try { r1 = arg1.toString().split(".")[1].length } catch (e) { r1 = 0 }
  try { r2 = arg2.toString().split(".")[1].length } catch (e) { r2 = 0 }
  m = Math.pow(10, Math.max(r1, r2));
...
//解决js运算精度丢失问题  乘法
export function jsMul(arg1, arg2) {
...
//解决js运算精度丢失问题  除法
export function jsDiv(arg1, arg2) {
  var t1 = 0, t2 = 0, r1, r2;
