...
if "%ip%" == "" (
 ...
 echo "开始发布Workflow到：%ip% ..."
)

set fromRoot=E:\DEV_CENTER\nannar\5I023_TIROS\sourcecode\1.trunk\tiros_workflow
set appPath=%fromRoot%
set appName=tiros_workflow
set copyLib=com-nannar-*.jar
...
if "%hour_ten%" == " " (
    set bakName=tiros_workflow_%date:~0,4%%date:~5,2%%date:~8,2%0%time:~1,1%%time:~3,2%.zip
) else (
    set bakName=tiros_workflow_%date:~0,4%%date:~5,2%%date:~8,2%%time:~0,2%%time:~3,2%.zip
)
