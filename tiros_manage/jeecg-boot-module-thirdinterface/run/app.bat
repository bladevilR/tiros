@echo off
set port=9096
set jarName=tiros_third_maximo.jar
set opration=%1
set pid=<nul
...
:fun_start
for /f "tokens=1,2 delims= " %%a in ('jps -l') do ( if "%%b"=="%jarName%" (set pid=%%a))
if DEFINED pid GOTO :isexist
start javaw -jar -Dfile.encoding=utf-8 -Dloader.path=resources,lib %jarName% --server.port=%port%
echo %jarName% 开始运行...
GOTO :end
