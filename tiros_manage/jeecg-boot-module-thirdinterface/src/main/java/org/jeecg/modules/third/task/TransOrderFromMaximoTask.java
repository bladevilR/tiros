...
@Component
@EnableScheduling
public class TransOrderFromMaximoTask {
    ...
    @Value("${serviceTask.transFromMaximo.order.enable}")
    private String enableTask;


    @Scheduled(fixedDelayString = "${serviceTask.transFromMaximo.order.interval}")
    private void checkAndDealTransData() {
        ...
        try {
            ...
            log.error("定时从maximo同步数据--工单：定时任务执行失败，原因如下：" + ex.getMessage());
