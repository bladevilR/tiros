...
@Component
@EnableScheduling
public class TransFaultFromMaximoTask {
    ...
    @Value("${serviceTask.transFromMaximo.fault.enable}")
    private String enableTask;


    @Scheduled(fixedDelayString = "${serviceTask.transFromMaximo.fault.interval}")
    private void checkAndDealTransData() {
        ...
        try {
            ...
            log.error("定时从maximo同步数据--故障：定时任务执行失败，原因如下：" + ex.getMessage());
