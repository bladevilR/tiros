...
public class BuMaterialGroupStock extends Model<BuMaterialGroupStock> {
    ...
    /**
     * 相关的退料明细id，后端用于处理数据变动
     */
    @ApiModelProperty(hidden = true)
    @TableField(exist = false)
    @JsonIgnore
    private Set<String> relativeReturnedDetailIdSet;

    @ApiModelProperty(value = "作业工单的车号，用于工单消耗填报的显示和比对")
    @TableField(exist = false)
    private String workOrderTrainNo;

    @ApiModelProperty(value = "作业工单的车号和班组库存的车号是否一致，用于工单消耗填报的显示和比对")
    @TableField(exist = false)
    @Dict(dicCode = "bu_state")
    private Integer trainNoIsSame;
