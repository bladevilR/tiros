...
public class BuPlanFormWorkRecordDetail extends Model<BuPlanFormWorkRecordDetail> {
    ...
    @ApiModelProperty(value = "如果作业记录表为新版（excel的）时，明细所在excel中的行数（从0开始的）, 默认-1")
    ...
    @TableField(exist = false)
    private String checkMaxStatus;

    @ApiModelProperty(value = "当前最高检查类型  字典dictCode=bu_work_order_record_check_type", notes = "1自检2互检3专检4抽检5过程检6过程检确认7交接检8交接检确认")
    @Dict(dicCode = "bu_work_order_record_check_type")
    @TableField(exist = false)
    private Integer currentMaxCheckType;
