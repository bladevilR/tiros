...
public class SysAnnouncementController {
     ...
     * @return
     */
    @RequiresPermissions(value = {"/isystem/annountCement"})
...
//        querySaWrapper.ge(SysAnnouncement::getEndTime, sysUser.getCreateTime()); //新注册用户不看结束通知
        // 新注册用户不看结束通知，修改为-->
        // 如果用户没有创建时间，不过滤时间；
        // 如果用户有创建时间，查询通知结束时间大于用户创建时间 或者 通知结束时间为空的数据
        if (null != sysUser.getCreateTime()) {
            querySaWrapper.and(wrapper ->
                    wrapper.ge(SysAnnouncement::getEndTime, sysUser.getCreateTime())
                            .or()
                            .isNull(SysAnnouncement::getEndTime));
        }
        // 换种过滤方式
...
//        }
        ...
        // 过滤
        announcements.removeIf(item -> anntIds.contains(item.getId()));
