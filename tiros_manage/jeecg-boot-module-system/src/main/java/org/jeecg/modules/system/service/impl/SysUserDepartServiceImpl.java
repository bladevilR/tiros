...
public class SysUserDepartServiceImpl extends ServiceImpl<SysUserDepartMapper, SysUserDepart> implements ISysUserDepartService {
    ...
    /**
     * @see ISysUserDepartService#listUserIdByDepId(String)
     */
    ...
    public List<String> listUserIdByDepId(String depId) {
        ...
        LambdaQueryWrapper<SysUserDepart> wrapper = new LambdaQueryWrapper<SysUserDepart>()
                .eq(SysUserDepart::getDepId, depId);
        List<SysUserDepart> userDepartList = this.list(wrapper);
        if (CollectionUtils.isNotEmpty(userDepartList)) {
            userIdList = userDepartList.stream()
