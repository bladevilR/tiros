package org.jeecg.modules.tiros.serialnumber.service.impl;
...
public class SerialNumberGenerateImpl extends ServiceImpl<SysSerialNumberMapper, SysSerialNumber> implements SerialNumberGenerate {
    ...
    /**
     * @see SerialNumberGenerate#generateSerialNumberByCode(String)
     */
    @Override
    ...
    /**
     * @see SerialNumberGenerate#generateOrderCode(String)
     */
    @Override
    public String generateOrderCode(String line) {
        if (StringUtils.isBlank(line)){
            throw new JeecgBootException("请选择工单所属线路");
        }

        String orderCode = generateSerialNumberByCode("JDXGD");

        String prefix = orderCode.substring(0, 1);
        String suffix = orderCode.substring(2);
