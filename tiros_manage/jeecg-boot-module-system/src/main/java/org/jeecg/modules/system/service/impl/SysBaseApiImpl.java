...
public class SysBaseApiImpl implements ISysBaseAPI {
    ...
    @Override
    public Map<String, String> mapAllUserIdRealName() {
        ...
        for (SysUser user : userList) {
            userIdNameMap.put(user.getId(), user.getRealname());
        }
    ...
    /**
     * @see ISysBaseAPI#listUsernameByTypeAndParam(int, List)
     */
    @Override
    public List<String> listUsernameByTypeAndParam(int type, List<String> paramList) {
        ...
        if (1 == type) {
            List<String> usernameList = sysUserDepartMapper.selectUserNameListByDepartIdList(paramList);
            usernameSet.addAll(usernameList);
        } else if (2 == type) {
            for (String roleCode : paramList) {
                List<String> usernameList = sysUserRoleMapper.selectUserNameListByRoleCode(roleCode);
                usernameSet.addAll(usernameList);
            }
        } else if (3 == type) {
            List<SysUser> sysUserList = sysUserMapper.selectBatchIds(paramList);
            List<String> usernameList = sysUserList.stream()
    ...
    @Override
    public String getDictItemTextByCodeAndValue(String code, String itemValue) {
        if (StringUtils.isBlank(itemValue)) {
            return "";
        }

        List<DictModel> dictModelList = queryDictItemsByCode(code);
        if (CollectionUtils.isEmpty(dictModelList)) {
            return "";
        }

        for (DictModel dictModel : dictModelList) {
            if (itemValue.equals(dictModel.getValue())) {
    ...
    public void sendSysAnnouncement(String fromUser, String toUser, String title, String msgContent, String setMsgCategory) {
        ...
        for (int i = 0; i < userIds.length; i++) {
            if (oConvertUtils.isNotEmpty(userIds[i])) {
                SysUser sysUser = sysUserMapper.selectUserByUsername(userIds[i]);
                if (sysUser == null) {
                    continue;
                }
                ...
                obj.put(WebsocketConst.CMD_TOPIC, announcement.getTitile());
    ...
    @Override
    public void sendScanCodeMsg(String toUser, String title, String msgContent) {

        if (oConvertUtils.isNotEmpty(toUser)) {
            SysUser sysUser = sysUserMapper.selectUserByUsername(toUser);
            if (sysUser == null) {
                return;
            }
            JSONObject obj = new JSONObject();
            obj.put(WebsocketConst.MSG_CMD, WebsocketConst.CMD_SCAN);
            obj.put(WebsocketConst.MSG_USER_ID, toUser);
            obj.put(WebsocketConst.CMD_TOPIC, title);
            obj.put(WebsocketConst.MSG_TXT, msgContent);
            webSocket.sendOneMessage(sysUser.getId(), obj.toJSONString());
        }
    }


    private void setUserDepartInfos(SysUser sysUser) {
