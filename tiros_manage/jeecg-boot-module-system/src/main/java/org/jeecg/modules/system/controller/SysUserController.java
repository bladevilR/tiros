...
public class SysUserController {
    ...
    public Result<IPage<SysUser>> queryPageList(SysUser user,
                                                @RequestParam(name = "depId", required = false) @ApiParam(name = "depId", value = "部门id，查此部门下人员") String depId,
                                                @RequestParam(name = "roleId", required = false) @ApiParam(name = "roleId", value = "角色id，查此角色的人员") String roleId,
        ...
        if (StringUtils.isNotBlank(depId)) {
            List<String> userIdList = sysUserDepartService.listUserIdByDepId(depId);
        ...
        if (StringUtils.isNotBlank(roleId)) {
            List<String> userIdList = sysUserRoleService.listUserIdByRoleId(roleId);
            if (CollectionUtils.isNotEmpty(userIdList)) {
                queryWrapper.in("id", userIdList);
            } else {
                queryWrapper.eq("id", "-1");
        ...
        IPage<SysUser> userPage = sysUserService.page(pageParam, queryWrapper);

        List<SysUser> userList = userPage.getRecords();
        if (CollectionUtils.isNotEmpty(userList)) {
            // 查询用户的所属部门
            List<String> userIdList = userList
                    .stream()
                    .map(SysUser::getId)
                    .distinct()
                    .collect(Collectors.toList());
            Map<String, String> useDepNames = sysUserService.getDepNamesByUserIds(userIdList);
            Map<String, String> userIdRoleNamesMap = sysUserService.getRoleNamesByUserIdList(userIdList);
            for (SysUser sysUser : userList) {
                //TODO 临时借用这个字段用于页面展示
                sysUser.setOrgCode(useDepNames.get(sysUser.getId()));
                // 设置部门id
                if (StringUtils.isNotBlank(depId)) {
                    sysUser.setDepartId(depId);
                } else {
                    List<SysDepart> userDeparts = sysDepartService.queryUserDeparts(sysUser.getId());
                    if (CollectionUtils.isNotEmpty(userDeparts)) {
                        sysUser.setDepartId(userDeparts.get(0).getId());
                    }
                }

                // 设置用户角色名称
                String roleNames = userIdRoleNamesMap.get(sysUser.getId());
    ...
    @RequiresPermissions(value = {"/isystem/user"})
    ...
    @RequiresPermissions(value = {"/isystem/user"})
    @ApiOperation(value = "重置密码", notes = "管理员不修改，工号非空的密码重置=工号@sz-mtr，工号为空的重置=用户名@sz-mtr")
    @PostMapping(value = "/resetPassword")
    public Result<String> resetPassword(@RequestParam(required = false) @ApiParam(value = "用户名，多个逗号分隔") String usernames,
                                        @RequestParam(required = false) @ApiParam(value = "更新所有用户（当用户名不传参时有用）") Boolean resetAll) throws Exception {
        String resetInfo = sysUserService.resetPasswordNew(usernames, resetAll);
        return new Result<String>().successResult(resetInfo);
    }
    ...
    @RequiresRoles({"admin"})
    @RequiresPermissions(value = {"/isystem/user"})
    @ApiOperation("修改用户密码（所有用户，如果密码为空则密码默认为：工号@sz-mtr）")
    @RequestMapping(value = "/changePassword/all", method = RequestMethod.PUT)
    public Result<?> changePasswordAll(@RequestBody SysUser sysUser) {
        // SysUser u = this.sysUserService.getOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getUsername, sysUser.getUsername()));
        List<SysUser> users = this.sysUserService.list(new LambdaQueryWrapper<SysUser>().ne(SysUser::getUsername, "admin"));
        for (SysUser user : users) {
            /*sysUser.setId(user.getId());*/
            if (StringUtils.isBlank(sysUser.getPassword())) {
                String pwd = user.getWorkNo() + "@sz-mtr";
    ...
    @ApiOperation(value = "根据部门id获取人员信息")
    public Result<List<SysUser>> queryUserByDepId(@RequestParam(name = "id", required = true) @ApiParam(name = "id", value = "部门id", required = true) String id,
                                                  @RequestParam(name = "realname", required = false) @ApiParam(name = "realname", value = "人员姓名") String realname) {
    ...
    public Result<?> changPassword(@RequestBody JSONObject json) {
        // 修改登录人员的密码，不应该传入用户名 String username = json.getString("username");
...
//                json.put("avatar", item.getSysUser().getAvatar());
    ...
    public Result<List<SysUser>> listByGroupId(@RequestParam(name = "groupId") @ApiParam(name = "groupId", value = "班组（工班）id") String groupId) throws Exception {
        List<SysUser> userList = sysUserService.listByGroupId(groupId);
        return new Result<List<SysUser>>().successResult(userList);
    }
