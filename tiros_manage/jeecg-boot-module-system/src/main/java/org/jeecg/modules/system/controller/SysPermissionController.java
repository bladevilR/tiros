...
public class SysPermissionController {
    ...
    public Result<List<SysPermissionTree>> list() {
        ...
        try {
            ...
            sortSysPermissionTree(treeList);
...
//	}
    ...
    public Result getUserPermissionByToken(@RequestParam(name = "token", required = true) String token) {
        ...
        try {
            ...
            List<SysPermission> metaList = sysPermissionService.queryByUser(username);
            metaList.sort(Comparator.comparing(SysPermission::getMenuType, Comparator.nullsLast(Comparator.naturalOrder()))
                    .thenComparing(SysPermission::getSortNo, Comparator.nullsLast(Comparator.naturalOrder())));
            //添加首页路由
            //update-begin-author:taoyan date:20200211 for: TASK #3368 【路由缓存】首页的缓存设置有问题，需要根据后台的路由配置来实现是否缓存
            if (!PermissionDataUtil.hasIndexPage(metaList)) {
                List<SysPermission> indexMenus = sysPermissionService.list(new LambdaQueryWrapper<SysPermission>().eq(SysPermission::getName, "首页"));
                if (indexMenus.size() > 0) {
                    metaList.add(0, indexMenus.get(0));
                }
            }
            ...
            allAuthList.sort(Comparator.comparing(SysPermission::getSortNo));
            JSONArray allauthjsonArray = new JSONArray();
            this.getAllAuthJsonArray(allauthjsonArray, allAuthList);
            //路由菜单
//            System.out.println("第一次");
//            System.out.println(menujsonArray.toString());
//            jsonArraySort(menujsonArray, "sortNo", false);
...
//            System.out.println(menujsonArray.toString());
     ...
     * @return
     */
    @RequiresPermissions(value={"/isystem/permission"})
     ...
     * @param treeList 菜单
     */
    private void sortSysPermissionTree(List<SysPermissionTree> treeList) {
        if (CollectionUtils.isNotEmpty(treeList)) {
            treeList.sort(Comparator.comparing(SysPermissionTree::getSortNo));
            for (SysPermissionTree sysPermissionTree : treeList) {
                List<SysPermissionTree> children = sysPermissionTree.getChildren();
                if (CollectionUtils.isNotEmpty(children)) {
                    sortSysPermissionTree(children);
                }
            }
        }
    }
