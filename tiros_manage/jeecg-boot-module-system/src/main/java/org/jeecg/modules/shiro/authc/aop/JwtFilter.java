...
import org.jeecg.common.exception.TokenExpireException;
	...
	protected boolean isAccessAllowed(ServletRequest servletRequest, ServletResponse servletResponse, Object mappedValue) {
		try {
			...
			if (e instanceof TokenExpireException) {
				...
				result.setResult(e);
				result.setCode(3166);
				result.setMessage("token已过期，请重新申请token");
				WebUtil.responseJsonResult(servletResponse, result, 401);
			} else {
				Result result = new Result<>();
				result.setResult(e);
				result.setCode(401);
				result.setMessage(e.getMessage());
				WebUtil.responseJsonResult(servletResponse, result, true);
			}
			return false;
			// throw new AuthenticationException("登录失败，请重新登录", e);
		}
	}
