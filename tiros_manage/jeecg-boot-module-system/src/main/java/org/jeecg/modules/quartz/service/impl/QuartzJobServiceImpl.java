...
public class QuartzJobServiceImpl extends ServiceImpl<QuartzJobMapper, QuartzJob> implements IQuartzJobService {
     ...
     */
    @Override
    public boolean pauseJob(QuartzJob quartzJob) throws SchedulerException {
    ...
    /**
     * @see IQuartzJobService#executeJobOnce(QuartzJob)
     */
    @Override
    public boolean executeJobOnce(QuartzJob quartzJob) throws Exception {
        JobKey jobKey = JobKey.jobKey(quartzJob.getJobClassName().trim());
        scheduler.triggerJob(jobKey);

        log.info("手动执行定时任务：" + quartzJob.getDescription() + " 一次");
        return true;
    }
    ...
    private void schedulerAdd(String jobClassName, String cronExpression, String parameter) {
        try {
            ...
            // 构建job信息
            JobDetail jobDetail = newJob(getClass(jobClassName).getClass()).withIdentity(jobClassName).usingJobData("parameter", parameter).build();
