...
@Slf4j
@Api(tags = "定时任务接口")
@RequiresPermissions(value = {"/isystem/QuartzJobList"})
public class QuartzJobController {
    ...
    @ApiOperation(value = "执行一次定时任务")
    public Result<Object> executeJobOnce(@RequestParam(name = "jobClassName") String jobClassName) throws Exception {
        QuartzJob job = quartzJobService.getOne(new LambdaQueryWrapper<QuartzJob>().eq(QuartzJob::getJobClassName, jobClassName));
        if (job == null) {
            return Result.error("定时任务不存在！");
        }
        boolean flag = quartzJobService.executeJobOnce(job);
        return Result.ok("执行一次定时任务成功");
    }
