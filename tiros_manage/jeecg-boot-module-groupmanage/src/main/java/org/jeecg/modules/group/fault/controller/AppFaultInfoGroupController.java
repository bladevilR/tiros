...
public class AppFaultInfoGroupController {

    private final BuFaultInfoGroupService buFaultInfoGroupService;

    public AppFaultInfoGroupController(BuFaultInfoGroupService buFaultInfoGroupService) {
        this.buFaultInfoGroupService = buFaultInfoGroupService;
    }


    @GetMapping("/page")
    @ApiOperation(value = "查询故障记录(分页)", notes = "查询当前人员班组的，需前端传入groupId=登录信息中返回的departId")
    @OperationLog()
  ...
  /*  @PostMapping("/add")
    @ApiOperation(value = "新增并提交故障")
    public Result<Boolean> add(@RequestBody @Validated BuFaultInfo buFaultInfo) throws Exception {
        boolean flag = buFaultInfoGroupService.saveAndSubmitFaultInfo(buFaultInfo);
    ...
    public Result<Boolean> add(@RequestBody @Validated BuFaultInfo buFaultInfo) throws Exception {
        boolean flag = buFaultInfoGroupService.saveFaultInfo(buFaultInfo);
        return new Result<Boolean>().successResult(flag);
    }
