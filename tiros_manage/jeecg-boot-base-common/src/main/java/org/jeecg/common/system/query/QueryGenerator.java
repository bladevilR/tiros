...
public class QueryGenerator {
    ...
    public static void installMplus(QueryWrapper<?> queryWrapper, Object searchObj, Map<String, String[]> parameterMap) {
		...
		*/
        ...
        // 加上注解了@TableField(exist = false)的字段，不拼接条件
        Set<String> cannotSearchFieldNameSet = new HashSet<>();
        Field[] fields = searchObj.getClass().getDeclaredFields();
        for (Field field : fields) {
            TableField tableField = field.getAnnotation(TableField.class);
            if (null != tableField && !tableField.exist()) {
                cannotSearchFieldNameSet.add(field.getName());
            }
        }
        ...
        for (int i = 0; i < origDescriptors.length; i++) {
            ...
            if (cannotSearchFieldNameSet.contains(name)) {
                continue;
            }
