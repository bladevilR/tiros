...
public class BuTpRepairPlanForms extends Model<BuTpRepairPlanForms> {
    ...
    @ApiModelProperty(value = "关联的车辆结构id  表示这个表单用来记录哪个设备类型的，可以不填，需要填写多份的话最好选择设备类型，好自动匹配同一设备类型的不同设备。如果是记录表单，且有设置设备类型，说明对设备进行了拆装，那么则需要记录换上换下的设备部件号（出厂编码），如果对应得表单有设备类型id，则需要带入到这边")
